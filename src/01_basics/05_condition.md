...
fn main() {
    ...
    match score {
        ...
        80..=89 => println!("良好"),
        60..=79 => println!("及格"),
        40..60 => println!("差一点了，努把力！(左闭右开，不含60)"),
        _ => println!("不及格"),
    }
}
```
...
- **输出**：`良好`
- **注意**：`..=` 包含边界；`..` 排除。
- 90..=100，表示 包含两端的范围。等价于「90 ≤ score ≤ 100」。
- 90..100，表示 左闭右开 的范围。等价于「90 ≤ score < 100」
...
先匹配模式（如 Some(x)），再判断条件（如 x % 2 == 0），两者都满足才会进入该分支。
...
fn main() {
    ...
    match num {
        Some(x) if x % 2 == 0 => println!("偶数：{}", x), // 模式匹配成功 + 满足if条件，才会进入该分支
        Some(x) => println!("奇数：{}", x), // 模式匹配成功，但上面条件不成立，才会进入该分支
        None => println!("无值"), // 模式不匹配 Some(x)时会进入该分支。
    }
}
```
...
#### Option<T> 枚举
Rust 标准库中定义了一个非常常用的泛型枚举：
这个枚举表示「一个值要么存在，要么不存在」。
Some(T)：表示有值（T 就是值的类型）
None：表示没有值

这就是 Rust 用来安全地替代「空指针」的设计。
```rust
enum Option<T> {
    None,
    Some(T),
...
```rust
fn main() {
    let a = Some(5);   // a 的类型是 Option<i32>
    let b: Option<i32> = None; // 没有值，但类型必须声明
...
使用 `@` 将匹配值绑定到变量。
@ 运算符的作用是：在匹配一个模式的同时，把被匹配的值绑定到一个变量上。
...
fn main() {
    ...
    match num {
        x @ 1..=5 => println!("范围内的数：{}（1~5）", x),
        x @ 6..=10 => println!("范围内的数：{}（6~10）", x),
        _ => println!("超出范围"),
    }
...
## 6. 多模式与 OR
Rust 里的 | 确实被称为 “OR 模式”（pattern OR）,| 在 match 中是「模式的或」。

简单来说，“如果匹配任意一个模式都算成功”。1 | 2，等价于：“匹配 1 或 匹配 2，任意一个成立都执行这个分支。”
...
匹配成功 → 执行 if let 块，并把 Some 里的值绑定给 x
匹配失败 → 执行 else 块（如果写了）

注意：这里不是“复制”操作，而是“模式匹配 + 绑定变量”。

在这里Some(x)既是枚举类型的成员 也是模式，
枚举成员 + 模式 = 解构匹配

在 Rust 中，模式是一种结构匹配规则，可以匹配值的形状，同时提取内部的数据。
换句话说：模式是一种“描述值结构的模板”，用来检查或解构值。
...
(x, y) 是模式，用来匹配值的结构，并绑定变量 x = 1, y = 2
...
# 重点总结
## 重点一、Rust 的 `if` 是一个表达式，会返回值。如果没有 `else`，编译会失败，因为表达式必须有值。
Rust 的 `if` 是一个表达式，可以返回一个值，因此可以用于变量赋值。这类似于其他语言的三元运算符，但更灵活。
